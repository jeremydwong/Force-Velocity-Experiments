function [height,stack]=run4segTorque(state_cur,P,varargin)
...
% stack.work_tor(:,1) = stack.tor(:
nj=4;
stack.powers(:,1) = - 0 ...
    + stack.tor(:,1) .* stack.state(:,1+nj);

stack.powers(:,2) = - stack.tor(:,2) .* stack.state(:,1+nj) ...
    + stack.tor(:,2) .* stack.state(:,2+nj);

stack.powers(:,3) = - stack.tor(:,3) .* stack.state(:,2+nj) ...
    + stack.tor(:,3) .* stack.state(:,3+nj);

stack.powers(:,4) = - stack.tor(:,4) .* stack.state(:,3+nj) ...
    + stack.tor(:,4) .* stack.state(:,4+nj);

for ijoint =1:4
    stack.works(:,ijoint) = cumtrapz(stack.t,stack.powers(:,ijoint));
end;
stack.work=sum(stack.works,2);
stack.x = x';
stack.y = y';
stack.cmy = cmy';
stack.cmx = cmx';
